  ...
  async updateMetricsTable(date, textsData) {
    ...
    for (const texts of textsData) {
      ...
      const updateResult = await db.query(`
        ...
        SET total_texts_delivered = $1, texts_updated_at = $2
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!date) {
    console.error('❌ Usage: node daily-texts-etl-postgresql.js YYYY-MM-DD');
    process.exit(1);
  }

  const etl = new DailyTextsETLPostgreSQL();
...
export { DailyTextsETLPostgreSQL };