...
export default function AccountMetricsTableMonthly() {
  ...
  const needsBackendCSMFiltering = timePeriod !== 'current_month';

  const { data: apiResponse, isLoading } = useQuery({
    queryKey: ['/api/account-metrics-monthly', timePeriod, selectedStatus, selectedRiskLevel, selectedTrendingRiskLevel, ...(needsBackendCSMFiltering ? [selectedCSMs] : [])],
    queryFn: async () => {
  ...
  const { sortedAccounts, paginatedAccounts, totalPages, summaryStats, uniqueCSMs, uniqueRiskLevels, uniqueStatuses, uniqueTrendingRiskLevels, uniqueRiskReasons, uniqueTrendingRiskReasons } = useMemo(() => {
    ...
    // CSM filter - only apply frontend filtering for current month
            ...
            <>
              ...
              <div className="bg-red-50 p-4 rounded-lg border">
                ...
                <div className="space-y-2">
                  <div className={`flex justify-between items-center ${timePeriod === 'current_month' && !comparisonMetrics ? 'w-full' : ''}`}>
                    ...
                    <span className="text-sm font-bold text-red-600">{(riskCounts.historical?.high || 0).toLocaleString()}</span>
                  </div>
                  <div className={`flex justify-between items-center ${timePeriod === 'current_month' && !comparisonMetrics ? 'w-full' : ''}`}>
                    <span className="text-xs text-gray-600">Trending</span>
                    <span className="text-sm font-bold text-red-600">{(riskCounts.trending?.high || 0).toLocaleString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-orange-50 p-4 rounded-lg border">
                ...
                <div className="space-y-2">
                  <div className={`flex justify-between items-center ${timePeriod === 'current_month' && !comparisonMetrics ? 'w-full' : ''}`}>
                    <span className="text-xs text-gray-600">Current Month</span>
                    <span className="text-sm font-bold text-orange-600">{(riskCounts.historical?.medium || 0).toLocaleString()}</span>
                  </div>
                  <div className={`flex justify-between items-center ${timePeriod === 'current_month' && !comparisonMetrics ? 'w-full' : ''}`}>
                    <span className="text-xs text-gray-600">Trending</span>
                    <span className="text-sm font-bold text-orange-600">{(riskCounts.trending?.medium || 0).toLocaleString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-lg border">
                ...
                <div className="space-y-2">
                  <div className={`flex justify-between items-center ${timePeriod === 'current_month' && !comparisonMetrics ? 'w-full' : ''}`}>
                    <span className="text-xs text-gray-600">Current Month</span>
                    <span className="text-sm font-bold text-green-600">{(riskCounts.historical?.low || 0).toLocaleString()}</span>
                  </div>
                  <div className={`flex justify-between items-center ${timePeriod === 'current_month' && !comparisonMetrics ? 'w-full' : ''}`}>
                    <span className="text-xs text-gray-600">Trending</span>
                    <span className="text-sm font-bold text-green-600">{(riskCounts.trending?.low || 0).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </>
